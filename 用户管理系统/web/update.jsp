                    ...
                    <div class="form-bottom">
                        <form role="form" action="${pageContext.request.contextPath}/updateUserServlet" method="post"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="username" placeholder="用户名(必填)..." value="${beforeUser.username}"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="email" placeholder="邮箱(必填)..." value="${beforeUser.email}"
                                       class="form-username form-control" id="email">
                            </div>

                            <div class="form-group">
                                <label class="sr-only">手机号</label>
                                <input type="text" name="phone" placeholder="手机号(必填)..." value="${beforeUser.phone}"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="nickname" placeholder="昵称..." value="${beforeUser.nickname}"
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="address" id="address"
                                    ...
                                    <option <c:if test="${beforeUser.address == '内蒙古'}">selected</c:if>>内蒙古</option>
                                    <option <c:if test="${beforeUser.address == '辽宁'}">selected</c:if>>辽宁</option>
                                    <option <c:if test="${beforeUser.address == '吉林'}">selected</c:if>>吉林</option>
                                    <option <c:if test="${beforeUser.address == '黑龙江'}">selected</c:if>>黑龙江</option>
                                    <option <c:if test="${beforeUser.address == '上海'}">selected</c:if>>上海</option>
                                    <option <c:if test="${beforeUser.address == '江苏'}">selected</c:if>>江苏</option>
                                    ...
                                    <option <c:if test="${beforeUser.address == '山东'}">selected</c:if>>山东</option>
                                    ...
                                    <option <c:if test="${beforeUser.address == '广东'}">selected</c:if>>广东</option>
                                    <option <c:if test="${beforeUser.address == '广西'}">selected</c:if>>广西</option>
                                    <option <c:if test="${beforeUser.address == '海南'}">selected</c:if>>海南</option>
                                    <option <c:if test="${beforeUser.address == '重庆'}">selected</c:if>>重庆</option>
                                    <option <c:if test="${beforeUser.address == '四川'}">selected</c:if>>四川</option>
                                    <option <c:if test="${beforeUser.address == '贵州'}">selected</c:if>>贵州</option>
                                    <option <c:if test="${beforeUser.address == '云南'}">selected</c:if>>云南</option>
                                    <option <c:if test="${beforeUser.address == '西藏'}">selected</c:if>>西藏</option>
                                    <option <c:if test="${beforeUser.address == '陕西'}">selected</c:if>>陕西</option>
                                    <option <c:if test="${beforeUser.address == '甘肃'}">selected</c:if>>甘肃</option>
                                    <option <c:if test="${beforeUser.address == '青海'}">selected</c:if>>青海</option>
                                    <option <c:if test="${beforeUser.address == '宁夏'}">selected</c:if>>宁夏</option>
                                    <option <c:if test="${beforeUser.address == '新疆'}">selected</c:if>>新疆</option>
                                    <option <c:if test="${beforeUser.address == '港澳台'}">selected</c:if>>港澳台</option>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="occupation" placeholder="职业..." value="${beforeUser.occupation}"
                                       style="height: 50px;width: 100%" id="occupation">
                            </div>

                            <div class="form-group">
                                <label class="sr-only">个人说明</label>
                                <textarea style="width: 100%" name="describes" placeholder="个人说明..." rows="3"
